...
def add_rect(x,y,width,height,stroke="rgb(0,0,0)",fill="none",strokewidth=2,font_size=20.0,inside=True,outside=False) :
	tlx = x;
	tly = y;
	brx = x+width;
	bry = y+height;
...
parser.add_argument('--draw-width',
	action='store', dest='draw_width', type=float, default="0.2",
...
add_rect(0, 0, robot.width, robot.height, stroke="rgb(200,200,150)", strokewidth=1,fill="rgb(255,255,240)",outside=True)
add_rect(robot.x_min, robot.y_min, robot.img_width, robot.img_height, 
	stroke = "rgb(100, 255, 100)", strokewidth=1, fill="rgb(240,255,240)")
...
add_rect(minx, miny, width=maxx-minx, height=maxy-miny, stroke="rgb(128,128,128)", strokewidth=0.5,inside=False,outside=True)
doc.save(outfile)
