...
def add_rect(x,y,width,height,stroke="rgb(0,0,0)",fill="none",strokewidth=9,font_size=20.0,inside=True,outside=False) :
    tlx = x;
    tly = y;
    brx = x+width;
    bry = y+height;
...
parser.add_argument('--bot-spec',
    ...
    help="Robot spec as a height, width, top_margin, side_margin")
...
parser.add_argument('--draw-width',
    action='store', dest='draw_width', type=float, default="2",
...
add_rect(minx, miny, width=maxx-minx, height=maxy-miny, stroke="rgb(128,128,128)", strokewidth=2,inside=False,outside=True)
doc.save(outfile)
